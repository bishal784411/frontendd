...
import { createBankDetails } from "@/api/bank";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createBankDetails({
        ...
        userId: Number(user.id), // make sure this exists in user object
        branch: "Patan",               // hardcoded branch
        additionalInformation: "Nothing ok"
      });
  
      toast.success("Profile and bank details updated successfully");
    } catch (error) {
      toast.error("Failed to update profile or bank details");
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {/* <h1>Upload your tax document</h1> */}
              <div className="flex items-center gap-2">
                <AlertCircle className="h-4 w-4 text-destructive" />
                
                <p className="text-sm text-destructive">Upload your tax document</p>
              </div>
              <label className="block cursor-pointer">
                <input
                  type="file"
                ...
                <div className="border rounded-lg p-4">
                  {profileData.panCardImage ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-48 bg-muted rounded hover:bg-muted/80 transition-colors">
                      <Upload className="h-8 w-8 text-muted-foreground mb-2" />
                      <span className="text-primary hover:underline">Upload Front Page of Document</span>
                    </div>
                  )}
                </div>
              </label>
